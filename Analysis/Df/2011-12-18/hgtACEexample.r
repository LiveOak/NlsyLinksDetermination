...
mzData <- as.matrix(subset(ds, R==1, selVars))
fsData <- as.matrix(subset(ds, R==.5, selVars))
asData <- as.matrix(subset(ds, R==.375, selVars))
hsData <- as.matrix(subset(ds, R==.25, selVars))
...
ACEModel <- mxModel("ACE", # Twin ACE Model -- Path Specification
  ...
  mxPath(from=c("A1","C1","E1"), to="HtSt1", arrows=1, free=TRUE, values=.6, label=c("a","c","e")),
  mxPath(from=c("A2","C2","E2"), to="HtSt2", arrows=1, free=TRUE,values=.6, label=c("a","c","e")),
