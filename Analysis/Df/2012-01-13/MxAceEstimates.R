...
# dvName1 <- "HtSt1"
# dvName2 <- "HtSt2"
# ExtractHeritabilitiesMxAce(read.csv(pathInput))

ExtractHeritabilitiesMxAce <- function( dsLm ) {
   #dsLm <- ds
   #dsSem <- subset(dsLm, Subject1Tag < Subject2Tag )  
   dsSem <- subset(dsLm, Subject1Tag < Subject2Tag & !is.na(Dv_1) & !is.na(Dv_2) & !is.na(R))
   #dsSem <- subset(dsLm, ID_1 <  ID_2)
   selVars <- c(dvName_1, dvName_2)
   ...
   if( nrow(mzData) > 0 & nrow(asData) > 0) {
   ...
   else if( nrow(mzData) > 0 & nrow(asData) <= 0) {
      ...
      famACEModel <- mxModel("famACE", fsModel, asModel, hsModel,
         mxAlgebra(expression=FS.objective + AS.objective + HS.objective, name="twin"),
         mxAlgebraObjective("twin")
      )
   }
   else if( nrow(mzData) <= 0 & nrow(asData) <= 0 ) {
      ...
      stop("The pattern of missing groups isn't accounted for by the ACE routine.")
   }
